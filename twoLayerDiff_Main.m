...
L = 0.068; %sample length cm
D1 = linspace(1e-3,10,100); %diffusion constants to test cm^2/minute
D2 = D1;
        ...
        for d = 1:size(depth,2)
            for n = 1:N
                F1(n) = ((1-(sqrt(D1(a)/D2(b))))/(1+(sqrt(D1(a)/D2(b))))...
                    ^n*(erfc(((((2*n)+1)*L)+((sqrt(D1(a)/D2(b)))...
                    *depth(d)))/(2*sqrt(D1(a)*t)))));
                F2 = sum(F1);
            end
            C2(a,b,d) = (((2*(sqrt(D1(a)/D2(b)))*C0)/...
